...
func TestKafkaClient(t *testing.T) {
	...
	t.Run("publish to topic without retry and dql", func(t *testing.T) {
		...
		handler.Handle(func(r *ConsumeRecord) error {
			if v := string(r.Underlying.Value); v != "r2" {
				return fmt.Errorf("unexpected value %v", v)
			}
			r.Ack()
			return nil
		...
		handler.Handle(func(r *ConsumeRecord) error {
			if v := string(r.Underlying.Value); v != "r3" {
				return fmt.Errorf("unexpected value %v", v)
			}
			r.Ack()
			return nil
	...
	t.Run("ensure correct behavior of enabling/disabling consumers", func(t *testing.T) {
		...
		if err = client.DisableConsumerTopic(ctx, topic2); err != nil {
			t.Fatal(err)
		}
	...
	R1Loop:
		...
		if err = client.EnableConsumerTopic(ctx, topic2); err != nil {
			t.Fatal(err)
		}
		<-r1Topic2ReceivedChan
		// Disable topic1 before moving on to r2
		if err = client.DisableConsumerTopic(ctx, topic1); err != nil {
			t.Fatal(err)
		}
	...
	R2Loop:
		...
		if err = client.EnableConsumerTopic(ctx, topic1); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Verify failure scenarios", func(t *testing.T) {
		...
		useDlq := false
		registerConsumerWithHandler(t, client, handler, topic, retries, useDlq)
		err = client.RegisterConsumerFunc(topic, retries, useDlq, func(record *ConsumeRecord) {})
	...
	t.Run("same retry/dlq topic with different consumer groups (clients)", func(t *testing.T) {
		...
		// Enable dlq consumption
		if err := client1.EnableDlqConsumption(); err != nil {
			t.Fatal(err)
		}
		if err := client2.EnableDlqConsumption(); err != nil {
			t.Fatal(err)
		}
		if err := client3.EnableDlqConsumption(); err != nil {
			t.Fatal(err)
		}
